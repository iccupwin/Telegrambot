...
def handle_text(message):
    ...
    if message.text == "For Teachers":
        user_markup = telebot.types.ReplyKeyboardMarkup(True, False)
        user_markup.row('English File', 'English Plus')
        user_markup.row('Solutions', 'Family and Friends')
        user_markup.row('/start')
        bot.send_message(message.from_user.id, "Выберите Practice Test", reply_markup=user_markup)


    if message.text == "English File":
        user_markup = telebot.types.ReplyKeyboardMarkup(True, False)
        user_markup.row('Elementary', 'Pre-Intermediate')
        user_markup.row('Intermediate', 'Upper-Intermediate')
        user_markup.row('Advanced')
        user_markup.row('/start')
        bot.send_message(message.from_user.id, "Выберите уровень", reply_markup=user_markup)
    
    if message.text == "Elementary":
        inline_keyboard = telebot.types.InlineKeyboardMarkup()
        url_test_button = telebot.types.InlineKeyboardButton("Test", constants.url_elementary_test)
        url_essay_button = telebot.types.InlineKeyboardButton("Audio", constants.url_elementary_audio)
        url_answ_button = telebot.types.InlineKeyboardButton("Answers", constants.url_answ_one)
        url_exp_button = telebot.types.InlineKeyboardButton("Explanations", constants.url_exp_one)
        inline_keyboard.add(url_test_button)
        inline_keyboard.add(url_essay_button)
    ...
    if message.text =="Test":
        photo = open('https://i0.wampi.ru/2019/11/12/image.png', 'rb')
